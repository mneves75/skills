...
it leaves review judgment to the selected engine. From the repository being reviewed,
use the helper's installed path. In a clone of this skill collection:
...
at xhigh reasoning only for an account-access failure. Primary reasoning overrides
leave the fallback at xhigh. Honor explicit engine/model choices; do not
...
An explicitly selected model other than the default Astra has no automatic Codex fallback.
Check the startup engine/model/thinking lines; shell configuration may override defaults.
Review bundles leave the machine for the selected provider. Authorize private-code disclosure
before sending; secret scanning does not classify business confidentiality.
