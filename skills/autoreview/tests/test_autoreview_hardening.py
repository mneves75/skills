...
''',
    ...
    def test_outputs_reject_repository_symlinks_to_external_files(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            root = Path(tempdir)
            repo = init_repo(root)
            outside = root / "outside.json"
            outside.write_text("external sentinel\n")
            link = repo / "report.json"
            link.symlink_to(outside)
            for option in ("output", "json_output", "status_output"):
                with self.subTest(option=option):
                    args = argparse.Namespace(output=None, json_output=None, status_output=None)
                    setattr(args, option, str(link))
                    with self.assertRaisesRegex(SystemExit, "must point outside"):
                        self.helper["reject_repo_output_paths"](args, repo)
                    self.assertTrue(link.is_symlink())
                    self.assertEqual(outside.read_text(), "external sentinel\n")

    def test_outputs_write_the_validated_expanded_home_path(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            root = Path(tempdir)
            repo = init_repo(root)
            home = root / "home"
            ...
            (repo / "~").mkdir()
            for option in ("output", "json_output", "status_output"):
                with self.subTest(option=option), mock.patch.dict(
                    os.environ, {"HOME": str(home), "USERPROFILE": str(home)},
                ):
                    args = argparse.Namespace(output=None, json_output=None, status_output=None)
                    setattr(args, option, f"~/{option}.json")
                    self.helper["reject_repo_output_paths"](args, repo)
                    destination = Path(getattr(args, option))
                    self.assertEqual(destination, home.resolve() / f"{option}.json")
                    self.helper["atomic_write_text"](destination, "report\n")
    ...
    @unittest.skipIf(os.name == "nt", "POSIX executable marker fixture")
    def test_submodule_snapshot_keeps_outer_repository_executable_boundary(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            root = Path(tempdir)
            repo = init_repo(root)
            nested = repo / "sub"
            ...
            oid = git(nested, "rev-parse", "HEAD").strip()
            git(repo, "update-index", "--add", "--cacheinfo", f"160000,{oid},sub")
            marker = root / "repository-git-executed"
            binary_dir = repo / "bin"
            binary_dir.mkdir()
            trusted_git = self.helper["resolve_command"]("git", repo)
            write_executable(binary_dir / "git", (
                f"#!{sys.executable}\n"
                "import os, sys\nfrom pathlib import Path\n"
                f"Path({str(marker)!r}).write_text('executed')\n"
                f"os.execv({trusted_git!r}, [{trusted_git!r}, *sys.argv[1:]])\n"
            ))
            with mock.patch.dict(os.environ, {"PATH": str(binary_dir) + os.pathsep + os.environ["PATH"]}):
                before = self.helper["source_tree_snapshot"](repo)
                self.assertFalse(marker.exists(), "snapshot executed the repository-owned Git binary")
                source.write_text("after\n")
                self.assertNotEqual(self.helper["source_tree_snapshot"](repo), before)
                self.assertFalse(marker.exists())
